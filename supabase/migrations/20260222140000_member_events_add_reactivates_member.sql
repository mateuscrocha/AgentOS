...
with resolved_events as (
  ...
  where me.event_type in ('GROUP_PARTICIPANT_ADD','GROUP_PARTICIPANT_LEAVE','GROUP_PARTICIPANT_REMOVE')
),
latest_event as (
...
set
  left_at = case
    when le.event_type = 'GROUP_PARTICIPANT_ADD' then null
    else coalesce(m.left_at, le.occurred_at)
  end,
  status = case
    when le.event_type = 'GROUP_PARTICIPANT_ADD' then 'active'
    else 'inactive'
  end
