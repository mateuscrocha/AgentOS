...
export function useGroupDashboard({ groupId, dateRange }: UseGroupDashboardOptions) {
  ...
  const { data: previousSuggestionsData } = useQuery({
    queryKey: ['group-ikigai-suggestions-previous', groupId, previousPeriodStartISO, previousPeriodEndISO],
    queryFn: async () => {
      const { data } = await supabase
        .from('messages')
        .select('content, message_type, id, created_at, member_id, sender_phone')
  ...
  const EQ_TOKENS: Record<string, string> = {
    'ai': 'ia',
    'ia': 'ia',
    'inteligencia': 'ia',
    'artificial': 'ia',
  ...
  const CONTEXT_GROUPS: Array<{ label: string; keys: string[] }> = [
    { label: 'Inteligência Artificial', keys: ['ia'] },
    { label: 'Vendas & CRM', keys: ['crm'] },
    { label: 'Dados & Banco', keys: ['dados'] },
    { label: 'Integrações & API', keys: ['integracoes'] },
    { label: 'Produto & UX', keys: ['produto'] },
    { label: 'Marketing & Campanhas', keys: ['marketing'] },
  ];

  const EXTRA_STOPWORDS = new Set(['entao','tipo','cara']);

  const whitelist: string[] = [];
  const blacklist: string[] = [];

  const applyWhiteBlackList = (t: string) => {
    if (blacklist.includes(t)) return false;
    if (whitelist.length > 0) return whitelist.includes(t);
    return true;
  };

  const computeTopicBlocks = (src: any | null) => {
    const items = (src?.keywords || []) as Array<{ term: string; count: number; participants?: number }>;
    const mapCounts: Map<string, number> = new Map();
    const mapParticipants: Map<string, Set<string>> = new Map();
    const mapTerms: Map<string, Set<string>> = new Map();
    items.forEach(it => {
      const raw = (it.term || '').toString();
      const t = normalize(raw);
      if (EXTRA_STOPWORDS.has(t)) return;
      const eq = EQ_TOKENS[t] || canonical(t);
      if (!applyWhiteBlackList(eq)) return;
      mapCounts.set(eq, (mapCounts.get(eq) || 0) + (it.count || 0));
      if (!mapParticipants.has(eq)) mapParticipants.set(eq, new Set());
      const participantsSize = it.participants || 0;
      for (let i = 0; i < participantsSize; i++) {
        mapParticipants.get(eq)!.add(`${eq}-p${i}`);
      }
      if (!mapTerms.has(eq)) mapTerms.set(eq, new Set());
      mapTerms.get(eq)!.add(raw);
    });

    const blocks = Array.from(mapCounts.entries()).map(([key, count]) => {
      const grp = CONTEXT_GROUPS.find(g => g.keys.includes(key));
      const label = grp ? grp.label : key.toUpperCase();
      const participants = (mapParticipants.get(key)?.size || 0);
      const terms = Array.from(mapTerms.get(key) || []).slice(0, 6);
      return { label, key, terms, count, participants };
    })
    .filter(b => b.count >= 3 && b.participants >= 2)
    .sort((a, b) => b.count - a.count)
    .slice(0, 20);
  ...
  const topicBlocks = computeTopicBlocks(suggestionsData || null);
  const previousTopicBlocks = computeTopicBlocks(previousSuggestionsData || null);
  ...
  return {
    ...
    previousIkigaiSuggestions: previousSuggestionsData || { themes: [], keywords: [] },
    topicBlocks,
    previousTopicBlocks,
    busyDayAvatars,
    peakWindowAvatars,
