...
DECLARE
  ...
  v_participant_is_super_admin boolean;
  v_participant_is_owner boolean;
  v_has_is_owner_column boolean;
BEGIN
  v_has_is_owner_column := EXISTS (
    SELECT 1
  ...
  IF jsonb_typeof(p_participants) = 'array' THEN
    ...
    LOOP
      ...
      IF left(v_participant_digits, 2) = '55' AND length(v_participant_digits) >= 10 THEN
        v_participant_phone_e164 := '+' || v_participant_digits;
      ELSE
        v_participant_phone_e164 := '+55' || v_participant_digits;
      END IF;

      v_participant_is_owner := COALESCE((v_participant->>'is_owner')::boolean, false);
      v_participant_is_super_admin := COALESCE((v_participant->>'is_super_admin')::boolean, false);
      v_participant_is_admin := COALESCE((v_participant->>'is_admin')::boolean, false) OR v_participant_is_owner OR v_participant_is_super_admin;

      IF v_has_is_owner_column THEN
        INSERT INTO public.members (
          ...
          v_participant_is_admin,
          v_participant_is_super_admin,
          v_participant_is_owner,
