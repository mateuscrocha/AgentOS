# ENAP / Programas de Subvenção / Governo Federal

## Contexto Inicial

- contato citado: `Vanessa`, da `ENAP`
- natureza da frente: oportunidade institucional/comercial ligada a programas de subvenção e incentivo à capacitação no governo federal
- tese inicial percebida: entender como o Bóris pode capturar investimento, entrar em programas aderentes e construir caminho de venda para governo
- estado atual: oportunidade em exploração inicial, com primeira reunião já consolidada e aguardando editais em PDF e materiais complementares

## Sinais Concretos Já Confirmados

- Vanessa atua na `ENAP`, dentro da diretoria de inovação (`Gnova`) e no laboratório de inteligência artificial
- a ENAP trabalha com `inovação aberta`, aproximando setor público e setor privado
- a conversa apontou `CPSI` como mecanismo relevante a estudar
- Vanessa citou `subvenção econômica` com exemplo ligado a `FINEP/MCTI`
- deve sair em breve um edital da `Semana de Inovação` da ENAP, evento previsto para `novembro`
- a plataforma `desafios.enap.gov.br` apareceu como canal importante do ecossistema
- Vanessa prometeu compartilhar uma apresentação feita para o `BNDES Garagem`
- o primeiro material recebido reforça que o `CPSI` e a formulação correta da dor pública são os eixos centrais de entrada

## Materiais Já Recebidos

- apresentação `B2G — Como vender para o setor público em um cenário de inovação e IA`
- leitura consolidada em `editais/2026-04-07-b2g-como-vender-para-o-setor-publico-cenario-inovacao-ia.md`
- portal `InovaGov / ENAP` analisado em `analises/2026-04-07-inovagov-enap-portal.md`

## Hipótese de Oportunidade

- pode existir um caminho indireto de entrada via capacitação, inovação, programa de fomento ou subvenção
- a oportunidade talvez envolva enquadramento do Bóris como tecnologia aplicável a treinamento, gestão, acompanhamento operacional ou inteligência para operações públicas
- a análise precisa separar com clareza:
- o que é edital de fomento
- o que é programa de capacitação
- o que é compra pública
- o que é parceria institucional ou piloto

## O Que Precisamos Confirmar

- qual programa, linha ou edital a Vanessa mencionou exatamente
- qual é o órgão pagador, operador ou patrocinador do recurso
- se `BODs` foi o termo correto ou se houve referência a outro ente, banco ou mecanismo
- quem pode ser proponente elegível
- se a entrada do Bóris seria:
- como fornecedor
- como executor de capacitação
- como parceiro tecnológico
- como startup beneficiária de subvenção
- quais restrições jurídicas, cadastrais e operacionais existem para vender ao governo nesse caminho

## Leitura Comercial Provisória

- esta não é uma oportunidade de venda consultiva comum
- o ciclo tende a exigir leitura institucional, documental e de elegibilidade antes de qualquer proposta comercial
- o maior risco agora é misturar linguagem comercial com linguagem de edital sem entender o mecanismo real
- o maior ativo agora é organizar bem a informação para decidir onde vale investir energia

## Próximos Passos

1. Receber os PDFs e salvar em `editais/`
2. Receber a apresentação prometida pela Vanessa
3. Extrair nomes exatos de programas, órgãos, linhas e prazos
4. Mapear critérios de elegibilidade, contrapartidas e tipo de recurso
5. Construir uma tese objetiva de como o Bóris pode usufruir da oportunidade da melhor forma
